...
pub mod pallet {
	...
	#[pallet::storage]
	#[pallet::getter(fn expert_application_uid_count)]
	...
	#[pallet::storage]
	#[pallet::getter(fn get_expert_application)]
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn apply_for_expert_role(origin: OriginFor<T>) -> DispatchResult {
			...
			let uid = Self::expert_application_uid_count().checked_add(1).ok_or(ArithmeticError::Overflow)?;
			...
			let vote_type = VoteType::ExpertVerification;
		...
		pub fn cast_vote_for_expert_role(origin: OriginFor<T>, vote_type: VoteType, voting_id: u64, vote_cast: bool) -> DispatchResult {
			...
			let mut member = Self::get_member(who.clone()).ok_or(Error::<T>::NotAMember)?;
			// Check Vote Type
			ensure!(vote_type == VoteType::ExpertVerification || vote_type == VoteType::ExpertCouncilApproval, Error::<T>::WrongVoteType);
			...
			if vote_type == VoteType::ExpertCouncilApproval {
